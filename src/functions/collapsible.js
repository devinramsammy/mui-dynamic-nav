...
import { Collapse, List, ListItem, ListItemText, ListItemIcon, Icon } from '@material-ui/core';
			...
			<Collapse in={open} timeout="auto" unmountOnExit>
				<List component="div" disablePadding>
					{children.map((nav, index) => {
						return (
							<>
								<ListItem button={true} key={index} className={styles.nestedList}>
									<ListItemIcon>
										<Icon color="primary">{nav.icon}</Icon>
									</ListItemIcon>

									<ListItemText primary={nav.title} className={styles.listItem} />
								</ListItem>
							</>
						);
					})}
